    ...
    <>
      {!loading && (
        <Layout title={`${frontmatter.title} - Agustin Ribotta`}>
          ...
          <div id="buttons" className="flex justify-between space-x-2 md:space-x-0 lg:space-x-3 h-[5rem] mx-auto md:my-10">
            <Button icon={ChevronLeftIcon} title={t("common:prev-button")} position="left" boolean={!!prevProject} />
            <div className="w-12 md:w-16 h-[.2rem] bg-black dark:bg-white rotate-[120deg] translate-y-8 rounded-md"></div>
            <Button icon={ChevronRightIcon} title={t("common:next-button")} position="right" boolean={!!nextProject} />
          </div>
        </Layout>
      )}
    </>
...
export const getStaticProps = async ({ params }) => {
  ...
  const actuallyProjectIndex = await allData.findIndex((post) => post.replace(/\.mdx/, "") === params.slug);
