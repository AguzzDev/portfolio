...
export const ListComponent = ({ data }) => {
  const { tec, lib, year, client, website, repo } = data
  const { t } = useTranslation()

  const TableChildrenData = ({ title, data }) => (
    <>
      {typeof data === "object" ? (
        data == "" ? (
          <p>No se usaron</p>
        ...
        <p>
          {title === "Client" || title === "Cliente" ? (
            <p>
              {client === "Propio"
                ? t("common:own-project")
                : t("common:client-project")}
            </p>
          ) : (
            data
          )}
  ...
  const TableChildren = ({ title, data, children, last }) => {
    return (
